...
def app_module(monkeypatch, temp_state_file):
    ...
    monkeypatch.setenv("EXPECTED_PANGOLIN_HEADER_KEY", "")
    monkeypatch.setenv("EXPECTED_PANGOLIN_HEADER_VALUE", "")
...
def test_banner_requires_remote_user_header(app_module):
    app = app_module
    with start_server(app.BannerHandler) as (httpd, port):
        ...
        body = resp.read()
        assert resp.status == 403, (resp.status, body)


def test_banner_serves_png_and_updates_state(app_module):
    app = app_module
    # sanity: ensure no headers expected
    assert app.EXPECTED_PANGOLIN_HEADER_KEY == ""
    assert app.EXPECTED_PANGOLIN_HEADER_VALUE == ""
...
def test_security_header_enforced(monkeypatch, temp_state_file):
    ...
    monkeypatch.setenv("EXPECTED_PANGOLIN_HEADER_KEY", "X-Test-Key")
    monkeypatch.setenv("EXPECTED_PANGOLIN_HEADER_VALUE", "v123")
    ...
    with start_server(app.BannerHandler) as (httpd, port):
        # Missing header -> 403
        conn = http.client.HTTPConnection("127.0.0.1", port, timeout=5)
        conn.request("GET", "/banner.png", headers={"Remote-User": "bob"})
        resp = conn.getresponse()
        _ = resp.read()
...
def test_security_header_misconfigured_only_key(monkeypatch, temp_state_file):
    # Only key is set -> should reject because both key and value must be configured and present
    monkeypatch.setenv("EXPECTED_PANGOLIN_HEADER_KEY", "X-Only-Key")
    monkeypatch.setenv("EXPECTED_PANGOLIN_HEADER_VALUE", "")
    monkeypatch.setenv("PANGOLIN_TOKEN", "")
    monkeypatch.setenv("RESOURCE_IDS", "8")
    monkeypatch.setenv("STATE_FILE", temp_state_file)
    ...
    with start_server(app.BannerHandler) as (httpd, port):
        # Even if we send the header, since value isn't configured, it should 403
        conn = http.client.HTTPConnection("127.0.0.1", port, timeout=5)
        headers = {"Remote-User": "bob", "X-Only-Key": "anything"}
        conn.request("GET", "/banner.png", headers=headers)
        resp = conn.getresponse()
        _ = resp.read()
        assert resp.status == 403
...
def test_security_header_misconfigured_only_value(monkeypatch, temp_state_file):
    # Only value is set -> should reject because key is unknown
    monkeypatch.setenv("EXPECTED_PANGOLIN_HEADER_KEY", "")
    monkeypatch.setenv("EXPECTED_PANGOLIN_HEADER_VALUE", "v123")
    monkeypatch.setenv("PANGOLIN_TOKEN", "")
    monkeypatch.setenv("RESOURCE_IDS", "8")
